...
// FD-IDE
async function makePromiseHostOnce(method, payload, callback) {
...
window.listenToHost('reload', (event, args) => {
  SDK.targetManager.enableEmulation();
})
...
SDK.TargetManager = class extends Common.Object {
  ...
  // FD-IDE
  async enableEmulation(t) {
    const target = t || this._targets[0];
    const { width, height } = await makePromiseHostOnce('getWebviewWidthHeight');
    ...
    await emulationAgent.setDeviceMetricsOverride(width, height, 0, true, false);
  }
   ...
   * @return {!SDK.Target}
   */
  async createTarget(name, capabilitiesMask, connectionFactory, parentTarget) {
